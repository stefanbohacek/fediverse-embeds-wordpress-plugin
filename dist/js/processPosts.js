import{fetchData}from"./fetchData.js";import{getPostData}from"./getPostData.js";import{renderPost}from"./renderPost.js";import{dispatchEvent}from"./dispatchEvent.js";const processPosts=()=>{const postsEmbeds=document.querySelectorAll("blockquote.ftf-fediverse-post-embed");let posts=[];for(const post of postsEmbeds){const postData=getPostData(post);posts.push(postData)}// console.log('post data', posts);
posts.length&&posts.forEach(post=>{fetchData({action:"ftf_get_post",post:JSON.stringify(post)},response=>{console.log("ftf_get_post",{post,response}),renderPost(response)})})};export{processPosts};/***************************************************************************************************************/ //TODO: Old code for handling post attachments, might be useful for non-Mastodon platforms.
